...
def choose_day_button():
	...
	if str(response) == 'True':
		P2()
		if day_selected.get() == "Monday":
			route_list = Mon_day_route_list
		elif day_selected.get() ==  "Tuesday":
			route_list = Tues_day_route_list
		elif day_selected.get() ==  "Wednesday":
			route_list = Wed_day_route_list
		elif day_selected.get() ==  "Thursday":
			route_list = Thurs_day_route_list
		elif day_selected.get() ==  "Friday":
			route_list = Fri_day_route_list
	else:
...
while x	< 51:
	...
	#t = str("RT" + str(x))
	t = Checkbutton(routing_frame, text=(x), onvalue="R", offvalue="DR")
	t.deselect()
